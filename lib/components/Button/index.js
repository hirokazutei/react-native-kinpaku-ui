...
function buttonFactory({ themes, sizes, additionalPalettes, defaultColor, defaultType = 'fill', }) {
    ...
    for (const variationKey of BUTTON_SHAPE_VARIATION_KEYS) {
        const Button = ({ _additionalButtonProps, _additionalButtonStyle, _additionalTextProps, _additionalTextStyle, align = DEFAULT_BUTTON_ALIGN, color = defaultColor, isDisabled, isStretched, onPress, size = 'default', label, type = defaultType, }) => {
            // Palettes
            ...
            return (<TouchableOpacity style={touchableStyle} disabled={isDisabled} onPress={onPress} accessibilityLabel={label} {..._additionalButtonProps || {}}>
