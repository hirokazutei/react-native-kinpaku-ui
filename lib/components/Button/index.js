...
function buttonFactory({ themes, sizes, additionalPalettes, defaultType = 'solid', allowAdditionalPalettes, }) {
    const paletteContext = React.createContext('default');
    const buttons = {};
    BUTTON_VARIATION_KEYS.forEach((variation) => {
        const Button = ({ color = 'primary', size = 'default', isDisabled, isStretched, align = DEFAULT_BUTTON_ALIGN, onPress, title, type = defaultType, additionalButtonProps, additionalButtonStyle, additionalTextProps, additionalTextStyle, }) => {
            // Palettes
