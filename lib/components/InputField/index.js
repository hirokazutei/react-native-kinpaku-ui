    ...
    for (const key in DEFAULT_INPUT_VARIATION_SETTING) {
        if (DEFAULT_INPUT_VARIATION_SETTING.hasOwnProperty(key)) {
            const InputField = (_a) => {
                ...
                const _b = DEFAULT_INPUT_VARIATION_SETTING[key], { fontFamily, isBold, isItalic, leftIcon, letterSpacing, lineHeight, maxLength: defaultMaxLength, returnKeyType, rightIcon, textAlign, textContentType } = _b, inputFieldOptions = __rest(_b, ["fontFamily", "isBold", "isItalic", "leftIcon", "letterSpacing", "lineHeight", "maxLength", "returnKeyType", "rightIcon", "textAlign", "textContentType"]);
                // WrappedStyle
