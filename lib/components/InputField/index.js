...
function inputFieldFactory({ themes, additionalPalettes, sizes, defaultColor, inputFieldType = 'outline', defaultShape = 'circular', }) {
    ...
    for (const settingKey in DEFAULT_INPUT_VARIATION_SETTING) {
        if (DEFAULT_INPUT_VARIATION_SETTING.hasOwnProperty(settingKey)) {
            const InputField = (_a) => {
                var { _additionalTextInputProps, _additionalTextInputStyle, _additionalWrapperProps, _additionalWrapperStyle, backgroundColor, borderColor, color = defaultColor, isDisabled, maxLength, size, shape, textColor } = _a, inputFieldProps = __rest(_a, ["_additionalTextInputProps", "_additionalTextInputStyle", "_additionalWrapperProps", "_additionalWrapperStyle", "backgroundColor", "borderColor", "color", "isDisabled", "maxLength", "size", "shape", "textColor"]);
                // Palettes
                ...
                const wrapperStyleProps = Object.assign({ flexDirection: 'row' }, (isDisabled ? { borderColor: disabledColor } : borderColorProp), (isDisabled
                    ...
                    : backgroundColorProp), borderWidthProp, borderBottomWidthProp, borderRadiusProp, paddingProp, paddingHorizontalProp, paddingVerticalProp, _additionalWrapperStyle);
                // FieldStyle
                const fieldStyleProps = Object.assign({ flex: 1 }, (isBold ? { fontWeight: 'bold' } : {}), (isItalic ? { fontStyle: 'italic' } : {}), (fontFamily ? { fontFamily } : {}), (isDisabled ? { color: disabledColor } : { color: primaryTextColor }), (textAlign ? { textAlign } : {}), { fontSize: sizeProp.fontSize }, (letterSpacing ? { letterSpacing } : {}), (lineHeight ? { lineHeight } : {}), _additionalTextInputStyle);
                return (<View style={wrapperStyleProps} {..._additionalWrapperProps}>
            {leftIcon}
            <TextInput style={fieldStyleProps} {...(isDisabled ? { editable: !isDisabled } : {})} {...(maxLength ? { maxLength } : defaultMaxLength)} {...inputFieldProps} {...inputFieldOptions} {..._additionalTextInputProps}/>
            {rightIcon}
          </View>);
            };
