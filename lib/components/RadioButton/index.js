...
function radioButtonFactory({ themes, sizes, additionalPalettes, defaultColor, defaultType, }) {
    // Context
    ...
    for (const variationKey of RADIO_BUTTON_SHAPE_VARIATION_KEYS) {
        const RadioButton = ({ _customInnerViewProps, _customInnerViewStyle, _customOuterViewProps, _customOuterViewStyle, active, color = defaultColor, isDisabled, onPress, size = 'default', type = defaultType, }) => {
            ...
            const ringBorderRadius = variationKey == 'Sharp' ? 0 : borderRadiusResolver(ringSize);
            const dotBorderRadius = variationKey == 'Sharp' ? 0 : borderRadiusResolver(dotSize);
            // Outer Ring Style
