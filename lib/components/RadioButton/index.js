...
import { RADIO_BUTTON_VARIATION_KEYS, DEFAULT_RADIO_BUTTON_SIZES, } from './constants';
function radioButtonFactory({ themes, sizes, additionalPalettes, }) {
    const themeContext = React.createContext('default');
    const radioButtons = {};
    RADIO_BUTTON_VARIATION_KEYS.forEach((variation) => {
        const Button = ({ active, color, isDisabled, onPress, size = 'default', _customInnerViewProps, _customInnerViewStyle, _customOuterViewProps, _customOuterViewStyle, }) => {
            // Palettes
            ...
            const sizeProperty = sizes
                ? sizes[size]
                : DEFAULT_RADIO_BUTTON_SIZES[size];
